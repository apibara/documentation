...
- the `factory` callback receives a block and returns an (optional) filter and an (optional) piece of data passed to the integration. If the factory returns a filter, this filter used to stream data that will be passed to the `transform` function. Filters from subsequent calls to `factory` are merged together.
