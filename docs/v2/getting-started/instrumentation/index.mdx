...
```bash
pnpm install @opentelemetry/api @opentelemetry/auto-instrumentations-node @opentelemetry/exporter-prometheus @opentelemetry/exporter-trace-otlp-http @opentelemetry/resources @opentelemetry/sdk-node @opentelemetry/sdk-trace-node @opentelemetry/semantic-conventions
```

#### 2. Configure OpenTelemetry

Create an `otel` directory with an `otel.ts` file to set up the OpenTelemetry SDK:
...
#### 3. Update instrumentation.ts to use the OpenTelemetry SDK
...
#### 4. Build and run your indexer to see the metrics and traces on your configured observability platform.
...
In Grafana, you can create dashboards to visualize metrics from your Apibara indexer. The following metrics are available out of the box:

- `current_block`: The latest block number being processed by the indexer
- `processed_block_count`: Total number of blocks that have been processed
- `reorg_count`: Number of chain reorganizations detected and handled
...
Create an `otel` directory with an `otel.ts` file to set up Sentry:
...
#### 3. Update instrumentation.ts to use Sentry

Update the `instrumentation.ts` file at the root of your project:

```ts [instrumentation.ts]
import type { RegisterFn } from "apibara/types";
import { setupSentry } from "./otel/otel";
...
#### 4. Build and run your indexer with Sentry error tracking enabled
