...
- compaction service: combines multiple blocks together into _segments_.
  Segments are grouped by data type (e.g. logs, transactions, and receipts).
...
- joins (e.g. include logs' transactions) are also achieved with indices.
...
1000) into _segments_. Segments only contain data for one fragment type (headers,
indices, transactions, and so on).
